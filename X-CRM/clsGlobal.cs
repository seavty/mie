...
namespace X_CRM
{
    public class clsGlobal
    {
        ...
        public string stockVerification(sapi.db db, sapi.sapi cls, string itemid, double qty, string whid, string init_InvoiceItemID = "")
        {
            string re = "";
            double exQty = 0;
            if (init_InvoiceItemID != "")
                exQty = db.cNum(db.readData("init_Qty",
                    "Select init_Qty From tblInvoiceItem Where init_InvoiceItemID=" + init_InvoiceItemID));
        ...
        public string stockDeduction(sapi.db db, string itemID, string warehouseID, double quit_Qty, string inid_invoicedetailid = "", string init_InvoiceItemID = "", bool add = false)
        {
        ...
        public void invoiceTotal(string eid, sapi.db db, double deposit = 0, string soid = "")
        {
            ...
            foreach (DataRow row in tbl.Rows)
            {
                ...
                vat = db.cNum(row["invo_VAT"].ToString());

                //--ty add
                //vatAmount = db.cNum(row["invo_VATAmount"].ToString());
            ...
            foreach (DataRow row in tbl.Rows)
            {
                ...
                vatAmount = vat / 100 * invo_Total ;  // ty addd

                //Tax Calculation
                invo_Tax = invo_Total * invo_IsTax / 100;
                //invo_GTotal = invo_Total + invo_Tax;

                invo_GTotal = invo_Total + vatAmount;// ty add
                ...
                rDeposit = 0; // ty add ; becos if dont use it blance will become 0
                var re = db.execData("Update tblInvoice Set " +
                    ...
                    ",invo_VATAmount = " + vatAmount +
                    (soid == "" ? "" : ",invo_Deposit = " + rDeposit) +
                    ",invo_Balance = " + (invo_GTotal - rDeposit - invo_CreditNote) + " - isNull(invo_Deposit,0) - isNull(invo_PaidAmount,0) " +
        ...
        public void validPO(string id, sapi.db db)
        {
            DataTable tbl = db.readData("select * from tblPurchaseOrderItem" +
                " where poit_RemainQty>0 and poit_PurchaseOrderID = " + id);
            if (tbl.Rows.Count <= 0)
            {
